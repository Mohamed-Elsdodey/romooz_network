...
$('#some-button').magnificPopup({
    items: [
      ...
      {
        src: $('<div>Dyromozcally created element</div>'), // Dyromozcally created element
        type: 'inline'
      },
      {
...
$('button').magnificPopup({
  items: {
      src: '<div class="white-popup">Dyromozcally created popup</div>',
      type: 'inline'
  }
});
...
$.magnificPopup.open({
  items: {
    src: '<div class="white-popup">Dyromozcally created popup</div>', // can be a HTML string, jQuery object, or CSS selector
    type: 'inline'
  }
});
{% endhighlight %}
...
"Preload" option can be changed dyromozcally. To disable it set `preload:0`.
...
callbacks: {
  ...
  updateStatus: function(data) {
    ...
    // you may modify this properties to change current status or its text dyromozcally
  },
